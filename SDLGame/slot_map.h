    ...
    class values_iterator
    {
      private:
        T* getCurrent() noexcept
        {
            ...
            ValueStorage& v = slotMap->getValueByAddr(addr);
            T* value = reinterpret_cast<T*>(&v);
            return value;
        }

      public:
        explicit values_iterator(slot_map* _slotMap, size_type index) noexcept
            : slotMap(_slotMap)
            , currentIndex(index)
        {
        }

        T& operator*() noexcept { return *getCurrent(); }
        T* operator->() noexcept { return getCurrent(); }

        bool operator==(const values_iterator& other) const noexcept
        {
        ...
        values_iterator& operator++() noexcept
        {
            do
            {
                currentIndex++;
        ...
        values_iterator operator++(int) noexcept
        {
